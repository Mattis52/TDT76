...
def midfile_to_piano_roll_ins(filepath, instrument_n=0, fs=5):
    """ convert mid file to piano_roll csv matrix, but selecting a specific instrument in the mid file
        input: path to mid file, instrument to select in midifile
        output: path to piano_roll csv file
    """
    pm = pretty_midi.PrettyMIDI(filepath)
...
def load_all_dataset(dirpath, binarize=True):
    """ given a directory finds all the csv in the directory an load them as numpy arrays
        input: path a directory
        output: list of numpy arrays
    """
    if binarize:
        return [(pd.read_csv(os.path.join(dirpath, file)).values > 0).astype(int) for file in
                sorted(os.listdir(dirpath)) if file.endswith(".csv")]
    else:
        return [pd.read_csv(os.path.join(dirpath, file)).values for file in sorted(os.listdir(dirpath)) if
                file.endswith(".csv")]


def load_all_dataset_names(dirpath):
    """ given a directory finds all the csv in the directory an split the first part
...
def get_max_length(dataset):
    """ find the maximum length of piano roll matrices in a list of matrices
        ...
        output: maximum shape[1] of the list of arrays
...
def gen_music_seconds(model, init, composer=0, fs=5, gen_seconds=10, init_seconds=5):
    if init is None:
        ...
        song = generate_round(model, torch.LongTensor([composer]).unsqueeze(1).cuda(),
                              (gen_seconds - init_seconds + 1) * fs, 1, init[1:(init_index + 1)])
